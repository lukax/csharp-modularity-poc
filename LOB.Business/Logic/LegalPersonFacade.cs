        ...
        public void ConfigureValidations()
        {
            ...
            if (_entity != null)
            {
                _entity.AddValidation((sender, name) => _entity.CorporateName.Length < 1 ?
                    new ValidationResult("CorporateName", Strings.Error_Field_Empty) : null);
                _entity.AddValidation((sender, name) => _entity.TradingName.Length < 1 ?
                    new ValidationResult("TradingName", Strings.Error_Field_Empty) : null);
                _entity.AddValidation((sender, name) => _entity.Cnpj.ToString().Length < 1 ?
                     new ValidationResult("Cnpj", Strings.Error_Field_Empty) : null);
            }
        }
