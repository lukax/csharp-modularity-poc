    ...
    public class CustomerFacade : ICustomerFacade
    {
        ...
        public void SetEntity<T>(T entity) where T : Customer
        {
            if (entity.PersonType == PersonType.Legal)
                _legalPersonFacade.SetEntity(entity.Person as LegalPerson);
            if (entity.PersonType == PersonType.Natural)
                _naturalPersonFacade.SetEntity(entity.Person as NaturalPerson);
            _entity = entity;
        }
        ...
        public void ConfigureValidations()
        {
            ...
            if (_entity != null)
            {
                _entity.AddValidation((sender, name) => _entity.CustomerOf.Count < 1 ?
                    new ValidationResult("CustomerOf", Strings.Error_Field_Empty) : null);
            }
        }
