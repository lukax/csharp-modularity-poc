...
namespace LOB.Business.Logic
{
    public class EmployeeFacade : IEmployeeFacade
    {
        ...
        public void ConfigureValidations()
        {
            ...
            if (_entity != null)
            {
                _entity.AddValidation((sender, name) => _entity.Title.Length < 1 ?
                    new ValidationResult("Title", Strings.Error_Field_Empty) : null);
                _entity.AddValidation((sender, name) => _entity.HireDate.ToShortDateString().ToString().Length < 1 ?
                    new ValidationResult("HireDate", Strings.Error_Field_Empty) : null);
            }
        }
