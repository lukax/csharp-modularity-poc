...
namespace LOB.Business.Logic
{
    public class NaturalPersonFacade : INaturalPersonFacade
    {
        ...
        public void ConfigureValidations()
        {
            ...
            if (_entity != null)
            {
                _entity.AddValidation((sender, name) => _entity.FirstName.Length < 1 ?
                    new ValidationResult("Name", Strings.Error_Field_Empty) : null);
                _entity.AddValidation((sender, name) => _entity.LastName.Length < 1 ?
                    new ValidationResult("Description", Strings.Error_Field_Empty) : null);
                _entity.AddValidation((sender, name) => _entity.Cpf.ToString().Length < 1 ?
                     new ValidationResult("Cpf", Strings.Error_Field_Empty) : null);
                _entity.AddValidation((sender, name) => _entity.Rg.ToString().Length < 1 ?
                     new ValidationResult("Rg", Strings.Error_Field_Empty) : null);
                _entity.AddValidation((sender, name) => _entity.BirthDate.ToShortDateString().ToString().Length < 1 ?
                    new ValidationResult("BirthDate", Strings.Error_Field_Empty) : null);
            }
        }
