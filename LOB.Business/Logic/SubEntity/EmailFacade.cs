...
namespace LOB.Business.Logic.SubEntity {
    ...
    public sealed class EmailFacade : BaseEntityFacade<Email>, IEmailFacade {
        ...
        public override Email GenerateEntity() {
            Email result = base.GenerateEntity();
            result.Value = "";
            return result;
        }
            ...
            AddValidation(delegate {
                              ...
                              if(
                                      !Regex.IsMatch(Entity.Value,
                                              @"^([\w-\.]+)@((\[[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}\.)|(([\w-]+\.)+))([a-zA-Z]{2,4}|[0-9]{1,3})(\]?)$")) return new ValidationResult("Value", Strings.Notification_Field_InvalidFormat);
                              return null;
                          });
        }
    }
