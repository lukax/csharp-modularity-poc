...
namespace LOB.Domain.Monetary {
    ...
    public class SimpleMoney : IComparable<SimpleMoney>, IEquatable<SimpleMoney>, IComparable {
        private static readonly CurrencyCodes currencyCode =
                (CurrencyCodes)Enum.Parse(typeof(CurrencyCodes), new RegionInfo(CultureInfo.CurrentCulture.LCID).ISOCurrencySymbol);
        #region Constructors
        ...
        public SimpleMoney(decimal amount)
                : this((double)amount) { }
        public SimpleMoney(double amount) { this._amount = amount; }
        ...
        public int CompareTo(object obj) {
            ...
            if(!(obj is SimpleMoney)) throw new ArgumentException("Argument must be SimpleMoney");
        ...
        public override bool Equals(object obj) { return (obj is SimpleMoney) && Equals((SimpleMoney)obj); }
        ...
        public static bool operator !=(SimpleMoney first, SimpleMoney second) { return !first.Equals(second); }

        public static bool operator >(SimpleMoney first, SimpleMoney second) { return first.Amount > second.Amount; }
        ...
        public static SimpleMoney operator +(SimpleMoney first, SimpleMoney second) { return new SimpleMoney(first._amount + second._amount); }
        ...
        public static implicit operator SimpleMoney(long amount) { return new SimpleMoney(amount); }

        public static implicit operator SimpleMoney(decimal amount) { return new SimpleMoney(amount); }

        public static implicit operator SimpleMoney(double amount) { return new SimpleMoney(amount); }
        ...
        public static bool operator !=(SimpleMoney money, long value) { return !(money == value); }

        public static bool operator ==(SimpleMoney money, decimal value) {
            if(ReferenceEquals(money, null) || ReferenceEquals(value, null)) return false;
            return (money.Amount == value);
        }
        public static bool operator !=(SimpleMoney money, decimal value) { return !(money == value); }

        public static bool operator ==(SimpleMoney money, double value) {
            if(ReferenceEquals(money, null) || ReferenceEquals(value, null)) return false;
            return (money.Amount == (decimal)value);
        }
        public static bool operator !=(SimpleMoney money, double value) { return !(money == value); }

        public static SimpleMoney operator +(SimpleMoney money, long value) { return money + (double)value; }
        public static SimpleMoney operator +(SimpleMoney money, decimal value) { return money + (double)value; }
        ...
        public SimpleMoney Copy() { return new SimpleMoney(Amount); }

        public SimpleMoney Clone() { return new SimpleMoney(); }

        public string ToString(string format = "C") { return Amount.ToString(format, Currency.Get(currencyCode).NumberFormat); }
        ...
        public SimpleMoney[] Allocate(int n) {
            ...
            var results = new SimpleMoney[n];
            var remainder = (int)((_amount * cents) % n);
            for(int i = 0; i < remainder; i++) results[i] = new SimpleMoney((Decimal)highResult);
            for(int i = remainder; i < n; i++) results[i] = new SimpleMoney((Decimal)lowResult);
            return results;
        }
