...
namespace LOB.UI.Core.netfx.System {
    /// <summary>
    ...
    internal static class StringFormatWithExtension {
        ...
        ///     Expression like {ViewState} are replaced with the corresponding
        ...
        private static string OutExpression(object source, string expression) {
            ...
            try {
                if(string.IsNullOrEmpty(format)) return (DataBinder.Eval(source, expression) ?? "").ToString();
                else return DataBinder.Eval(source, expression, "{0:" + format + "}") ?? "";
            } catch(HttpException) {
